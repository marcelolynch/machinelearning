...
class BinaryNaiveBayesClassifier:
    ...
    def fit(self):
        ...
        for k,v in self.classified.items():
            ...
            must_correct = 0 in self.classified[k][:-1] # Frequency 0 for attribute in this class
            if must_correct:
                break         

            self.priors[k] = v / self.counts[k]
            self.priors[k] = np.append(self.priors[k], self.counts[k] / self.n_examples) # Last element is the frequency of the class
